...
def main():
    builder = SimulationBuilder(SimulationConfiguration(duration=120, debug=True, real_time=True))
    builder.add_handler(CommunicationHandler())
    builder.add_handler(TimerHandler())
    builder.add_handler(MobilityHandler())
    builder.add_handler(VisualizationHandler())

    builder.add_node(LeaderProtocol, (0, 0, 4))
    builder.add_node(FollowerProtocol, (-2, -2, 2))
