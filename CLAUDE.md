...
8. **`communication_protocol` must match across the fleet.** It selects the message-API transport — `"http"` (default, HTTP/1.1 over TCP via uvicorn), `"https"` (HTTP/1.1 over TLS via uvicorn), or `"http3"` (QUIC/HTTP3 via Hypercorn, requires `pip install "gradys-embedded[http3]"`). Mixed transports cannot talk to each other. Unrelated to `uav_api`'s own `--udp` flag — the local `uav_api` connection is always plain HTTP on `localhost`.
...
- **`RunnerConfiguration`** (`gradys_embedded/runner/configuration.py`) — `node_id`, `node_ip_dict`, `origin_gps_coordinates`, `initial_position`, `uav_api_port`, `telemetry_interval`, `communication_protocol` (+ optional `certfile`/`keyfile`).
- **Message API** (`gradys_embedded/runner/message_api.py`) — a FastAPI app with `POST /message` that each node runs on its own port. Served over HTTP/TCP (uvicorn) by default, HTTPS/TLS (uvicorn) when `communication_protocol="https"`, or QUIC/HTTP3 (Hypercorn) when `communication_protocol="http3"`; same app and payload in every mode.
