...
7. **`node_ip_dict` is the peer directory and cannot be changed at runtime.** Every node must already know every other node's `ip:port`. There is no discovery protocol — **except** `communication_protocol="zenoh"` with `auto_scout=True`, which uses Zenoh multicast scouting to discover peers automatically (requires a multicast-capable LAN).
8. **`communication_protocol` must match across the fleet.** It selects the inter-node data-plane transport — `"http"` (default, HTTP/1.1 over TCP via uvicorn), `"https"` (HTTP/1.1 over TLS via uvicorn), `"http3"` (QUIC/HTTP3 via Hypercorn, requires `pip install "gradys-embedded[http3]"`), or `"zenoh"` (Eclipse Zenoh pub/sub in peer/p2p mode, requires `pip install "gradys-embedded[zenoh]"`; discovery via `auto_scout`). Mixed transports cannot talk to each other. Unrelated to `uav_api`'s own `--udp` flag — the local `uav_api` connection is always plain HTTP on `localhost`.
9. **`auto_scout` only does something for `zenoh`.** `True` = Zenoh multicast scouting (auto-discovery); `False` (default) = explicit peer endpoints from `node_ip_dict`. It is accepted but **ignored** for http/https/http3 (a warning is logged).
...
- **Message API** (`gradys_embedded/runner/message_api.py`) — split into `create_control_app` (`/protocol/*` control plane, always plain HTTP on `control_api_port`) and `create_message_app` (`/message` data plane, on the `node_ip_dict` port). The data plane is served over HTTP/TCP (uvicorn) by default, HTTPS/TLS or QUIC/HTTP3 per `communication_protocol`; when `communication_protocol="zenoh"` there is no `/message` server — a Zenoh peer session carries messages instead.
